...
public final class MWController implements MainWindow.Callback {
    ...
    public static JSONObject loadEmailLogin()
        throws IOException,
               JSONException {
        final File file = new File("email.key");
        JSONObject data = null;
        // try (final InputStream is = new FileInputStream(file)) {
        //     final String pwd = "/909s=mlad7=oskn&4[___x77s(&saDXxhv";
        //     final Key key = Key.fromInputStream(is);
        //     data = new JSONObject(MergeKeyOpener.openPublicKey(key, pwd));
        // }
        return data;
    }
    ...
    public MWController(User user, StringResources sr, Callback callback) {
        this.user = user;
        this.sr = sr;
    ...
    public void saveEmailInfo(
        ...
        try {
            dao.save(data, Paths.get("email.json"));
        }
        catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public LinkedList<String> listCurrentTerms() throws SQLException {
        ...
        final String sql = "SELECT t.name FROM current_term JOIN "
                           + "term t ON current_term.term_id = t.id";
        try (
    ...
    public JSONArray listTerms() throws Exception {
        final URL url = new URL(user.getWebsiteURL().toString() + "/term.php");
        final HttpURLConnection connection =
            (HttpURLConnection) url.openConnection();
        final byte[] postDataBytes = "param=list".getBytes(
            ...
            try {
                ...
                for (Row row : sheet) {
                    ...
                    if (row.getRowNum() == 0) {
                        ...
                        currentPasswordRow.createCell(0).setCellValue("EMAIL");
                    ...
                    currentPasswordRow.createCell(0).setCellValue(currentEmail);
                    currentPasswordRow.createCell(1)
                ...
                final String msg =
                    sr.get(Strings.VAL_MISSING_REG_PROFESSORS) + i;
                connection.rollback();
        ...
        try (Connection connection = user.connect()) {
            ...
            try {
                ...
                for (Row row : sheet) {
                    ...
                    currentCampus = getIdFromCell(campuses, currentCampusCell);
                ...
                final String msg = sr.get(Strings.VAL_MISSING_REG_STUDENTS) + i;
                connection.rollback();
    ...
    public void updateStudents(
        ...
        try (Connection connection = user.connect()) {
            ...
            try {
                ...
                final String msg = sr.get(Strings.VAL_MISSING_UPD_STUDENTS) + i;
                connection.rollback();
                throw new SQLException(msg);
            }
            catch (Exception e) {
    ...
    public void deleteStudents(Sheet sheet, TaskDialog<Void> td)
        ...
        td.update(i);
        try (final Connection connection = user.connect()) {
            connection.setAutoCommit(false);
            try {
                Cell currentIdCell;
                int currentId;
                ...
                final String msg = sr.get(Strings.VAL_MISSING_DEL_STUDENTS) + i;
                connection.rollback();
                throw new Exception(msg);
            }
            catch (SQLException e) {
    ...
    public void downloadStudents(
        ...
        try (
            ...
            loadHashMaps(connection, campuses, careers);
            try (Workbook workbook = new XSSFWorkbook()) {
                final Map<Integer, String> campusesInv =
                    getInverseMap(campuses);
                ...
                while (resultSet.next()) {
                    ...
                    currentRow.createCell(3).setCellValue(currentSecondCareer);
    ...
    public void finishTerm(String term) throws Exception {
        ...
        // TODO ADD NEW IMPL update database stuff
        // final String selectSQL = "SELECT campus_id, career_id, results FROM "
        ...
        //         JSONObject currentCareerData = null;
        ...
        //             currentCampus = campuses.get(resultSet.getInt
        //             ("campus_id"));
        //             currentCareer = careers.get(resultSet.getInt
        //             ("career_id"));
    ...
    private void uploadResults(
        ...
        final String params =
            "param=term&name=" + name + "&data=" + termData.toString();
    ...
    private void registerStudent(
        Connection connection, int id, int campus, int career, int secondCareer,
    ...
    static ArrayList<Item> loadItems(String table, Connection connection)
        throws SQLException {
    ...
    private static Integer getIdFromCell(Map<String, Integer> map, Cell cell) {
    ...
    private static void registerProfessor(
        ...
        final String hasedPassword = BCrypt.hashpw(password, BCrypt.gensalt());
        ps.setString(1, name);
